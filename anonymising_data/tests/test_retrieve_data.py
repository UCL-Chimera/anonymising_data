...
def test_write_data(config):
    d = RetrieveData(config)
    d.write_data()
    newfile = Path(__file__).parent.parent.joinpath('tests/output/omop_data.csv')
    testfile = Path(__file__).parent.parent.joinpath('tests/resources/test_expected_omop.csv')
